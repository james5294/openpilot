...
def get_routes_names(footage_path: str) -> List[str]:
  route_times = [
    segment.route_name.time_str for segment in get_all_segment_names(footage_path)
  ]
  return sorted(set(route_times), reverse=True)

def get_segments_in_route(route_time_str: str, footage_path: str) -> List[str]:
  segment_names = [
    segment for segment in get_all_segment_names(footage_path) if segment.time_str == route_time_str
  ]
  return [f"{segment.time_str}--{segment.segment_num}" for segment in segment_names]
...
def segment_to_segment_name(data_dir: str, segment: str) -> SegmentName:
  fake_dongle = "FakeDongleID1337"
  return SegmentName(str(os.path.join(data_dir, f"{fake_dongle}|{segment}")))
