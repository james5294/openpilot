    ...
    window.app = {
        ...
        showSwitchConfirm: function(forkName) {
            ...
            if (!isCompatible && forkAgnos !== "unknown" && state.deviceAgnosVersion !== "unknown") {
                if (agnosCached) {
                    // AGNOS is cached - can flash locally (fast). Device-update DISABLED when AGNOS mismatches.
                    modal.open("⚠️ AGNOS Update Required", "<div style=\\"background: var(--op-bg-elevated); border-radius: 8px; padding: 12px; margin-bottom: 16px;\\"><p style=\\"margin: 0; font-size: 13px;\\">Current: <strong>AGNOS " + escapeHtml(state.deviceAgnosVersion) + "</strong> → Required: <strong>AGNOS " + escapeHtml(forkAgnos) + "</strong></p></div><div style=\\"background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3); border-radius: 8px; padding: 16px; margin-bottom: 16px;\\"><p style=\\"margin: 0; color: #22c55e;\\"><strong>✓ Cached and ready</strong></p><p style=\\"margin: 8px 0 0 0; color: var(--op-text-secondary);\\">Flash from cache for a safe switch (2-5 min)</p></div>", [{ label: "Cancel", onclick: "modal.close()" }, { label: "Flash from Cache", cls: "btn-primary", onclick: "app.switchFork('" + escapeHtml(forkName) + "', 'cache')" }]);
                } else if (agnosInvalid) {
                    var missingText = agnosMissing.join(", ");
                    // Device-update DISABLED when AGNOS mismatches - must repair cache first
                    modal.open("⚠️ AGNOS Cache Invalid", "<div style=\\"background: var(--op-bg-elevated); border-radius: 8px; padding: 12px; margin-bottom: 16px;\\"><p style=\\"margin: 0; font-size: 13px;\\">Current: <strong>AGNOS " + escapeHtml(state.deviceAgnosVersion) + "</strong> → Required: <strong>AGNOS " + escapeHtml(forkAgnos) + "</strong></p></div><div style=\\"background: rgba(255,140,0,0.1); border: 1px solid rgba(255,140,0,0.3); border-radius: 8px; padding: 16px; margin-bottom: 16px;\\"><p style=\\"margin: 0; color: #ff8c00;\\"><strong>Cache incomplete</strong></p><p style=\\"margin: 8px 0 0 0; color: var(--op-text-secondary);\\">Missing files: " + escapeHtml(missingText) + "</p></div><div style=\\"background: var(--op-bg-elevated); border-radius: 8px; padding: 12px;\\"><p style=\\"margin: 0; color: var(--op-text-secondary);\\">Repair the cache to enable switching.</p></div>", [{ label: "Cancel", onclick: "modal.close()" }, { label: "Repair Cache", cls: "btn-primary", onclick: "modal.close(); app.prepareAgnos('" + escapeHtml(forkName) + "', '" + escapeHtml(forkAgnos) + "');" }]);
                } else {
                    // AGNOS not cached - Device-update DISABLED, must prepare OS first
                    modal.open("⚠️ AGNOS Not Cached", "<div style=\\"background: var(--op-bg-elevated); border-radius: 8px; padding: 12px; margin-bottom: 16px;\\"><p style=\\"margin: 0; font-size: 13px;\\">Current: <strong>AGNOS " + escapeHtml(state.deviceAgnosVersion) + "</strong> → Required: <strong>AGNOS " + escapeHtml(forkAgnos) + "</strong></p></div><div style=\\"background: rgba(255,140,0,0.1); border: 1px solid rgba(255,140,0,0.3); border-radius: 8px; padding: 16px; margin-bottom: 16px;\\"><p style=\\"margin: 0; color: #ff8c00;\\"><strong>Not cached</strong></p><p style=\\"margin: 8px 0 0 0; color: var(--op-text-secondary);\\">Download AGNOS first to enable switching</p></div><div style=\\"background: var(--op-bg-elevated); border-radius: 8px; padding: 12px;\\"><p style=\\"margin: 0; color: var(--op-text-secondary);\\">Prepare OS downloads AGNOS and enables safe switching.</p></div>", [{ label: "Cancel", onclick: "modal.close()" }, { label: "Prepare OS", cls: "btn-primary", onclick: "modal.close(); app.prepareAgnos('" + escapeHtml(forkName) + "', '" + escapeHtml(forkAgnos) + "');" }]);
                }
            } else {
